...
import { apiAddNotify } from "../../../Service/Notify/Notify";
import { apiGetUser } from "../../../Service/User/User";

const CreateNotify = ({ onclose }) => {
  ...
  const handleAddNotify = async () => {
    if(title === "" && descript === ""){
      toast.error("Vui lòng nhập tên và mô tả thông báo!");
      return;
    }
    const data = {
    ...
    try {
      const response = await apiAddNotify(data);
...
<select
            ...
            <div className="flex mt-4 items-center justify-around">
              <button onClick={handleAddNotify} className="bg-red-600 hover:bg-red-700 font-bold text-white cursor-pointer rounded-lg text-sm px-10 py-3   ">
                {" "}
                Gửi thông tin
              </button>
            </div>
