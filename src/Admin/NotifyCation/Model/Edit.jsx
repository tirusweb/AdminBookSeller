...
import {
  apiGetNotifyByID,
  apiUpdateNotify,
} from "../../../Service/Notify/Notify";

const UpdateNotify = ({ onclose, id }) => {
  ...
  const fetchNotify = async () => {
    try {
      const response = await apiGetNotifyByID(id);
      console.log(response.data);
      if (response.data.status === 1) {
        setTitle(response.data.user.title);
        setDescription(response.data.user.descript);
      } else {
      }
    } catch (error) {
      setError(error);
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await apiUpdateNotify(id, data);
      if (response.data.status === 1) {
        await fetchNotify();
        ...
        <div className="bg-white overflow-y-auto rounded shadow-lg">
          ...
          <div className=" w-[500px] rounded mt-[60px] h-auto mb-10">
            ...
            <div className="flex mt-4 items-center justify-around">
              <button onClick={handleUpdate} className="bg-red-600 hover:bg-red-700 font-bold text-white cursor-pointer rounded-lg text-sm px-10 py-3   ">
                {" "}
                Lưu thay đổi
              </button>
            </div>
